...
export default class MiningApp extends React.Component {
  ...
  checkInputValuePrefix(inputValue) {
    ...
    if (
      userInputValue.startsWith("SFXt") ||
      userInputValue.startsWith("Safex")
    ) {
      if (
    ...
    return (
      <div className="mining-app-wrap">
        ...
        <div className="main animated fadeIn">
          ...
          <button
            className="button-shine instructions-btn"
        ...
        <div
          className={
            this.state.instructions_modal_active
              ? "modal instructions-modal active"
              : "modal instructions-modal"
          }
        >
          <span className="close" onClick={this.closeInstructionsModal}>
            X
          </span>
          ...
          {this.state.instructions_lang === "english" ? (
            <div>
              ...
              <p>
                ...
                <button>new wallet</button> button. In the dialog box, click{" "}
                <button>generate new wallet</button> which will create new Safex
                Wallet. Be sure to
                <button className="red-btn red-btn-en">
                  save wallet keys
                </button>{" "}
                before proceeding.
              </p>
              <p>
                <strong>
                  Wallet keys are made to control your coins, make sure you keep
                  them safe at all times. If your keys get stolen it can and
                  will result in total loss of your Safex Cash.
                </strong>
              </p>
              <p className="warning green">
                Once your wallet keys are saved, you are ready to start mining.{" "}
                <button className="green-btn">wallet keys saved</button>
              </p>
              <p>
                Enter you wallet address in the Safex Address field, select one
                of the pools you want to connect to, choose how much CPU power
                you want to use for mining and click start to begin. That's it,
                mining will start in a couple of seconds. Good luck!
              </p>
            </div>
          ) : (
            <div>
              <h3>Uputstvo</h3>
              <p>
                Ako nemate Safex Wallet, kliknite <button>new wallet</button>{" "}
                dugme. U dialog prozoru kliknite{" "}
                <button className="gen-new-wallet-sr">
                  generate new wallet
                </button>{" "}
                dugme koje će kreirati novi Safex Wallet. Obavezno sačuvajte
                Vaše ključeve{" "}
                <button className="red-btn">save wallet keys</button> pre nego
                što nastavite.
              </p>
              <p>
                <strong>
                  Ovi ključevi kontrolišu Vaše novčiće, zato ih uvek čuvajte na
                  bezbednom. Ako Vaši ključevi budu ukradeni sigurno ćete
                  izgubiti sav Vaš Safex Cash.
                </strong>
              </p>
              <p className="warning green">
                Sačuvajte Vaše ključeve, i spremni ste da počnete sa rudarenjem.{" "}
                <button className="green-btn">wallet keys saved</button>
              </p>
              <p>
                Ukucajte adresu Vašeg wallet-a u predviđeno polje, izaberite
                pool na koji želite da se povežete, izaberite koliku procesorku
                snagu želite da koristite i počnite sa rudarenjem. To je to,
                rudarenje će početi za par sekundi. Srećno!
              </p>
            </div>
          )}
        </div>
        ...
        <div
          className={
            this.state.modal_active || this.state.instructions_modal_active
              ? "backdrop active"
              : "backdrop"
          }
          onClick={
